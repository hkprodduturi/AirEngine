...
export function generateApp(ctx: TranspileContext, analysis: UIAnalysis): string {
  ...
  if (ctx.hasBackend && analysis.hasPages) {
    if (useLazy) {
      for (const page of analysis.pages) {
        ...
        lines.push(`const ${pageName}Page = lazy(() => import('./pages/${pageName}Page.js'));`);
      }
    } else {
      for (const page of analysis.pages) {
        const pageName = capitalize(page.name);
  ...
  if (ctx.hasBackend) {
    ...
    if (flags.hasTable) lines.push("import DataTable from './components/DataTable.js';");
    if (flags.hasIteration) lines.push("import EmptyState from './components/EmptyState.js';");
    if (flags.hasStat) lines.push("import StatCard from './components/StatCard.js';");
  }
  lines.push('');
