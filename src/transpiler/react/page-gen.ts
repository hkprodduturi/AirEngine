...
export function generatePageComponents(
  ...
  for (const page of analysis.pages) {
    ...
    const isAuthPage = page.name === 'login' || page.name === 'register' || page.name === 'signup';
    ...
    if (isShallowForm && !hasSidebar) {
      ...
      // Form nested in card/grid → just center, no extra card (page defines its own container)
      lines.push('    <div className="flex items-center justify-center min-h-screen p-4">');
      const childJsx = page.children.map(c =>
      ...
      const childJsx = page.children.map(c =>
        generateJSX(c, ctx, analysis, sidebarScope, 6)
      ).filter(Boolean).join('\n');
      lines.push(childJsx);
      lines.push('    </div>');
    } else {
